sub NONGMOVE {
	...
	if($t_mes eq ""){&ERR("주민을 이주시킬 도시가 없습니다.");}
...
<br>
$zname성으로부터 주민이주가능 지역<BR>
$t_mes
<br><br>
<b>주민을 파견하고자 하는 성의 아이콘을 클릭해주세요.</b><br>
<center>
...
<font color="white" face="돋움"><span style="font-size:9pt;">자신의 도시의 농민을 다른 도시로 이주시킵니다.<br>자신의 주민이주가능한 거리를 숙지하신 후 자신이 있는 성 주위로 사방 8블럭안에 있는 성을 이동할 수 있습니다.<br>자신의 통솔력*100만큼의 주민들을 이동시킬 수 있습니다.<br><font color=red>주민을 보낼 도시의 가용주민인구보다 많을 경우 이주가 되지 않습니다.</font></span></font></td>
...
<td width="30%" align="center"><span style="font-size:9pt;">주민의 수</span></td>
<td width="30%" align="center"><span style="font-size:9pt;">주민이주가능</span></td>
<td><span style="font-size:9pt;">주민이주숫자</span></td>
</tr>
<tr bgcolor=$TD_C3>
<td>
<p align="center"><span style="font-size:9pt;">주민이주</span></td>
<td><p align="center"><span style="font-size:9pt;">$znum명</span></td>
<td>
...
<table>
    <tr>
        <td>
<input type=hidden name=mode value=70>
<input type=hidden name=id value=$kid>
<input type=hidden name=pass value=$kpass>
<input type=hidden name=no value=$in{'no'}>
<input type=hidden name=num value=$in{'num'}>
<input type=submit value="이주한다"></form>
        </td>
        <td>
<center>
<form action="$FILE_STATUS" method="post">
...
sub NONGMOVE3 {
	...
	if($in{'gold'} <= 0 && $in{'rice'} <= 0){&ERR("0명은 안됩니다.");}
	...
	if($in{'no'} eq "all"){
		while(@NEW_COM_DATA < $MAX_COM){
			push(@NEW_COM_DATA,"70<>$gungold<>$town_name[$num] $in{'rice'}명 주민이주[금:$gungold]<>$tt<>$in{'gold'},$in{'rice'}<>$in{'num'}<><>\n");
		}
		$no = $in{'no'};
	}else{
		foreach(@COM_DATA){
			...
			foreach(@no){
				if($i eq $_){
					$ahit=1;
					push(@NEW_COM_DATA,"70<><>$town_name[$num] $in{'rice'}명 주민이주[금:$gungold]<>$tt<>$in{'gold'},$in{'rice'}<>$in{'num'}<><>\n");
					$lno = $_ + 1;
					$no .= "$lno,";
				}
			}
	...
	print <<"EOM";
<CENTER><hr size=0><h2>$town_name[$num]성으로 주민을 이주 시켰습니다. [소모금액:$gungold]</h2><p>
<form action="$FILE_STATUS" method="post">
<input type=hidden name=id value=$kid>
<input type=hidden name=pass value=$kpass>
<input type=hidden name=mode value=STATUS>
