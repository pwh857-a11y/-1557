sub YAKIUCHI2 {
	...
	if($in{'num'} eq ""){&ERR("방화지역을 입력하지 않았습니다.");}
	&CHARA_MAIN_OPEN;
	&TOWN_DATA_OPEN("$kpos");
	&TIME_DATA;
	$num = $in{'num'};
	...
	if($in{'no'} eq "all"){
		while(@NEW_COM_DATA < $MAX_COM){
			push(@NEW_COM_DATA,"38<><>$town_name[$num]성을 방화<>$tt<><>$in{'num'}<><>\n");
		}
		$no = $in{'no'};
	}else{
		foreach(@COM_DATA){
			...
			foreach(@no){
				if($i eq $_){
					$ahit=1;
					push(@NEW_COM_DATA,"34<><>$town_name[$num]성을 방화<>$tt<><>$in{'num'}<><>\n");
					$lno = $_ + 1;
					$no .= "$lno,";
				}
			}
	...
	print <<"EOM";
<CENTER><hr size=0><h2>$town_name[$num]성의 방화를 입력했습니다.</h2><p>
